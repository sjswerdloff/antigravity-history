...
def export(
    ...
    with Progress() as progress:
        ...
        with ThreadPoolExecutor(max_workers=MAX_WORKERS) as pool:
            futures = {pool.submit(_fetch_one, cid, info): cid for cid, info in sorted_items}
...
def list_conversations(
    ...
    summaries, merged, _ = get_all_trajectories_merged(endpoints)
...
def recover(
    ...
    indexed, merged, _ = get_all_trajectories_merged(endpoints)
...
def info(
    ...
    endpoints = _discover_endpoints(port, token)
    summaries, cascade, _ = get_all_trajectories_merged(endpoints)
